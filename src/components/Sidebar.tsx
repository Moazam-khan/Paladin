      ...
      <Row style={{ marginTop: 36 }} gutter={[0, 12]}>
        {menuItems.map((item) =>
          <Col
            ...
            style={{
              padding: 8,
              borderBottom: `1px solid ${activeMenu === item ? colors.primary : colors.white10}`,
            }}>
            <Text style={{ fontSize: 18, color: activeMenu === item ? colors.primary : colors.white50 }}>
              {item}
            </Text>
          </Col>)}
      </Row>
      <div style={{ position: 'absolute', bottom: 25, marginRight: 25, width: '-webkit-fill-available' }}>
        <Row style={{ marginTop: 40 }}>
          <Button secondary block size={'large'}>Disconnect wallet</Button>
        </Row>
